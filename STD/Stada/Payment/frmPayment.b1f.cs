    ...
    class frmPayment : UserFormBase
    {
        ...
        private void LoadDataGridCreate(string requestID = "")
        {
            ...
            if (this.grData != null)
            {
                ...
                this.grData.Columns.Item("DocEntry").ColumnConfig(string.Empty, false, false);
            ...
            try
            {
                ...
                if (data != null && !string.IsNullOrEmpty(data.PaymentID))
                {
                    try
                    {
                        ...
                        query += "\"Message\" = 'Đã gửi sang ngân hàng' ";
        ...
        private void TransferResultBIDV(string body)
        {
            ...
            try
            {
                UIHelper.LogMessage($"Bắt đầu gửi yêu cầu truy vấn kết quả thanh toán đến ngân hàng",
                    UIHelper.MsgType.StatusBar, false);
                var options = new RestClientOptions(APIBIDVConstrant.APILink);
                var client = new RestClient(options);
                var request = new RestRequest(APIBIDVConstrant.TransferInqBIDV + "?scope=read &JWE=Yes", Method.Post);
                request.AddHeader("Content-Type", "application/json");
                request.AddHeader("Timestamp", DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss.fffzzz"));
                request.AddHeader("Channel", APIBIDVConstrant.ChannelBIDVAPI);// ConfigurationManager.AppSettings["ChannelBIDVAPI"]);
                request.AddHeader("User-Agent", APIBIDVConstrant.UserAgentIDVAPI);// ConfigurationManager.AppSettings["UserAgentIDVAPI"]);
                ...
                string payload = "{\"body\": { \"listPaymentId\": " + JsonSerializer.Serialize(body) + " }}";
                ...
                var rps = JsonSerializer.Deserialize<BIDVTransfeInqResponses>(result);
                if (rps == null)
                {
                    UIHelper.LogMessage($"Lỗi không có phản hồi, vui lòng check lại", UIHelper.MsgType.StatusBar, true);
                    return;
                ...
                if (datas != null )
                {
                    try
                    {
                        foreach (var data in datas.data)
                        {
                            ...
                            var query = "UPDATE \"" + DIConnection.Instance.CompanyDB + "\".\"tb_Bank_TransferRecord\" ";
                            query += "SET \"bankStatus\" = '" + data.stateCode + "' ,";
                            query += "\"Message\" = '" + (data.message??"") + "' ";
                            query += "WHERE \"requestId\" = '" + data.reff + "' ";
                            query += "AND \"transId\" = '" + data.msgId + "' ";
                            query += "AND \"bank\" = 'BI' ";
                           ...
                           // status = item.status;
                            ...
                            //var query = "UPDATE \"" + DIConnection.Instance.CompanyDB + "\".\"tb_Bank_TransferRecord\" ";
                            //query += "SET \"Message\" = '" + data.msgId??"" + "' ";
                            //query += "WHERE \"requestId\" = '" + data.msgId + "' ";
                            //query += "AND \"bank\" = ''BI' ";
                        }
        ...
        private void UpdateBanksStatus(List<int> data)
        {
            if (Bank == Banks.ViettinBank.GetDescription())
            {
                foreach (var index in data)
                {
                    ...
                    UpdateVTBBankTransferStatus(index, ref status, ref requestID, ref date);
                    var message = string.Empty;
                    var isReturn = false;
            ...
            {
                var listPaymentId = string.Empty;
                foreach (var index in data)
                {
                    if (!string.IsNullOrEmpty(listPaymentId))
                    {
                        listPaymentId += ",";
                    }
                    var transId = this.grData.DataTable.GetValue("transId", index).ToString();
                    listPaymentId += transId;
                }

                TransferResultBIDV(listPaymentId);
                foreach (var index in data)
                {
                    var message = string.Empty;
                    var isReturn = false;
                    var status = this.grData.DataTable.GetValue("BankStatus", index).ToString();
                    var requestID = this.grData.DataTable.GetValue("requestId", index).ToString();
                    ...
                    //var key = $"PM{DateTime.Now.ToString("yyyymmddhhmms")}";
                    UIHelper.LogMessage("Bắt đầu tạo thanh toán trên SAP", UIHelper.MsgType.StatusBar, false);
                    var ret = PaymentViaDI.CreatePayment(dataPayment, requestID, string.Empty, ref message);
                    UIHelper.LogMessage(ret.Message, UIHelper.MsgType.StatusBar, !ret.Flag);
                }
            }
            LoadDataGridCreate();
        ...
        private void UpdateVTBBankTransferStatus(int index, ref string status, ref string requestID, ref string receiveDate)
        {
            try
            {
                status = "00";
